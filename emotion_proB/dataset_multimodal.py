    ...
    def __init__(self, df, text_folder, tokenizer, max_len=128):
        ...
        for _, row in self.df.iterrows():
            ...
            if raw_emotion not in EMOTION_MAP:
                continue # 매핑에 없는 감정(Fear, Disgust 등)은 건너뜀 (이미 필터링 했겠지만 안전장치)
...
def load_data_frames(session_folder):
    ...
    # 🔥 [수정] Stratify 기준을 5진 분류 라벨로 변경
    # EMOTION_MAP에 있는 감정만 남기고 매핑
    grouped = grouped[grouped['Emotion'].str.lower().isin(EMOTION_MAP.keys())]
    grouped["target"] = grouped["Emotion"].apply(lambda x: EMOTION_MAP[x.lower()])
